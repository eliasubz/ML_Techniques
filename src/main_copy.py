...
def run_experiment_portable(
    ...
    min_observations_ibl3: int = 4,
    confidence_z_ibl3: float = 0.674,
    ir_k: int = 3,
    ...
    for fold_id, (train_matrix, test_matrix) in enumerate(splits):
        ...
        if model is not Models.SVM:
            ...
            # This assumes your IBL.fit handles the direct string strategy names.
                ...
                ibl.fit(
                    train_matrix,
                    instance_red="IBL3",
                    min_observations=min_observations_ibl3,
                    confidence_z=confidence_z_ibl3,
                )
        ...
        if model is Models.FW_K_IBL:
            ...
            row = create_ir_ibl_csv_row(
                ...
                ir_k=ir_k if instance_reduction_strategy in [] else None,
                z_score_ibl3=confidence_z_ibl3 if instance_reduction_strategy in ["IBL3", "IBL3_verbose"] else None,
                min_observations_ibl3=min_observations_ibl3 if instance_reduction_strategy in ["IBL3", "IBL3_verbose"] else None,
                ...
                percentage_memory_reduction=(ibl.cp_before_ir - ibl.cp_after_ir)
                / ibl.cp_before_ir
                * 100,
            )
        ...
        # Convert current result to a one-row DataFrame
        df_row = pd.DataFrame([row])  # assumes `row` is a dict or list matching your columns

        # Append the row (create header only if file doesn’t exist yet)
        write_header = not out_csv.exists()
        df_row.to_csv(out_csv, mode="a", header=write_header, index=False)

        print(f"Saved fold {fold_id + 1}/{NUM_SPLITS} for {dataset_name}-{model.name.lower()} to {out_csv}")
